...
class StdQuotes(BaseQuotes):
    ...
    @staticmethod
    def _page_size(value, maximum=800):
        value = int(value)
        if value <= 0:
    ...
    def quotes_batch(self, symbol=None, batch_size=80, **kwargs):
        ...
        :param symbol: 股票代码或股票代码列表
        :param batch_size: 每批数量，最大 80
        :return: pd.DataFrame
        """
        ...
        for start in range(0, len(symbol), batch_size):
            batch = symbol[start:start + batch_size]

            try:
                markets = get_stock_markets(batch)
                result = self.client.get_security_quotes(markets)
            except ValidationException:
                return to_data(None)
        ...
        return to_data(results, symbol=symbol, client=self, **kwargs)

    def quote_depth(self, symbol=None, batch_size=80, **kwargs):
        """
        查询五档盘口。
        ...
        data = self.quotes_batch(symbol=symbol, batch_size=batch_size, **kwargs)
    ...
    def bars_all(self, symbol='000001', frequency=9, page_size=800, max_pages=200, sort=True, **kwargs):
        """
        分页获取全量 K 线数据。

        :param symbol: 股票代码
        :param frequency: K 线频率
        :param page_size: 每页数量，最大 800
        :param max_pages: 最大页数
        :param sort: 是否按时间升序排序
        :return: pd.DataFrame
        """
        ...
        while max_pages is None or page < max_pages:
            ...
            result = self.client.get_security_bars(int(frequency), int(market), str(symbol), int(start), int(page_size))
            ...
            raise RuntimeError('bars_all reached max_pages before the server returned a short page')

        data = to_data(results, symbol=symbol, client=self, **kwargs)
    ...
    def index_bars_all(self, symbol='000001', frequency=9, page_size=800, max_pages=200, sort=True, **kwargs):
        """
        分页获取指数全量 K 线数据。
        ...
        while max_pages is None or page < max_pages:
            start = page * page_size
            result = self.client.get_index_bars(int(frequency), int(market), str(symbol), int(start), int(page_size))
            ...
            raise RuntimeError('index_bars_all reached max_pages before the server returned a short page')
    ...
    def transactions(self, symbol='', start=0, offset=800, date='20170209', **kwargs):
        ...
        data = data.assign(
            date=date,
            datetime=pd.to_datetime(date + ' ' + data['time']),
            code=str(symbol),
            order=range(start, start + len(data.index)),
        )
        return data.set_index('datetime', drop=False, inplace=False)

    def transactions_all(self, symbol='', date='20170209', page_size=800, max_pages=100, sort=True, **kwargs):
        """
        分页获取指定日期全日历史分笔成交。
        ...
        while max_pages is None or page < max_pages:
            start = page * page_size
            data = self.transactions(symbol=symbol, start=start, offset=page_size, date=date, **kwargs)
            ...
            raise RuntimeError('transactions_all reached max_pages before the server returned a short page')
