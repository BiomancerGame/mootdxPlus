...
_MASK32 = 0xFFFFFFFF
# TDX GBBQ files use a fixed XOR table; this table follows the public pytdx/easy_tdx decoding approach.
_HEX_KEY = (
...
_BIN_KEYS = bytes.fromhex(_HEX_KEY.replace(' ', ''))


def read_gbbq(filepath):
    path = Path(filepath)
    if not path.is_file():
        raise FileNotFoundError(path)

    if path.suffix.lower() == '.csv':
        return _normalize_gbbq_frame(pd.read_csv(path))

    content = path.read_bytes()
    if len(content) < 4:
        return _normalize_gbbq_frame(pd.DataFrame())
    ...
    for _ in range(count):
        if offset + 29 > len(content):
            break
        clear_data = bytearray()
        for _round in range(3):
            eax = struct.unpack('<I', _BIN_KEYS[0x44:0x48])[0]
            ebx = struct.unpack('<I', content[offset:offset + 4])[0]
            num = (eax ^ ebx) & _MASK32
            numold = struct.unpack('<I', content[offset + 4:offset + 8])[0]

            for j in range(0x40, 0x04 - 1, -4):
                ebx = (num & 0xFF0000) >> 16
                eax = struct.unpack('<I', _BIN_KEYS[ebx * 4 + 0x448:ebx * 4 + 0x44C])[0]
                ebx = num >> 24
                eax_add = struct.unpack('<I', _BIN_KEYS[ebx * 4 + 0x48:ebx * 4 + 0x4C])[0]
                eax = (eax + eax_add) & _MASK32
                ebx = (num & 0xFF00) >> 8
                eax_xor = struct.unpack('<I', _BIN_KEYS[ebx * 4 + 0x848:ebx * 4 + 0x84C])[0]
                eax = (eax ^ eax_xor) & _MASK32
                ebx = num & 0xFF
                eax_add = struct.unpack('<I', _BIN_KEYS[ebx * 4 + 0xC48:ebx * 4 + 0xC4C])[0]
                eax = (eax + eax_add) & _MASK32
                eax_xor = struct.unpack('<I', _BIN_KEYS[j:j + 4])[0]
                eax = (eax ^ eax_xor) & _MASK32
                ebx = num
                num = (numold ^ eax) & _MASK32
                numold = ebx

            numold_op = struct.unpack('<I', _BIN_KEYS[:4])[0]
            numold = (numold ^ numold_op) & _MASK32
            clear_data.extend(struct.pack('<II', numold, num))
            offset += 8

        clear_data.extend(content[offset:offset + 5])
        market, code_raw, date_int, category, liutong_before, total_before, liutong_after, total_after = struct.unpack(
            '<B7sIBffff',
            clear_data,
        )
        rows.append(
            {
                'market': market,
                'code': code_raw.rstrip(b'\x00').decode('utf-8', errors='ignore'),
                'datetime': int(date_int),
                'category': int(category),
                'liutong_before': liutong_before,
                'total_before': total_before,
                'liutong_after': liutong_after,
                'total_after': total_after,
            }
        )
        offset += 5

    return _normalize_gbbq_frame(pd.DataFrame(rows))


def _normalize_gbbq_frame(data):
    columns = [
        'market',
        'code',
        'datetime',
    ...
    aliases = {
        'panqianliutong': 'liutong_before',
        'panhouliutong': 'liutong_after',
        'qianzongguben': 'total_before',
        'houzongguben': 'total_after',
        'liquidity_before': 'liutong_before',
        'liquidity_after': 'liutong_after',
        'shares_before': 'total_before',
        'shares_after': 'total_after',
    }
    data = data.rename(columns={key: value for key, value in aliases.items() if key in data.columns}).copy()
    if 'datetime' not in data.columns and 'date' in data.columns:
        data['datetime'] = pd.to_datetime(data['date']).dt.strftime('%Y%m%d').astype(int)
    if 'date' not in data.columns and 'datetime' in data.columns:
        data['date'] = pd.to_datetime(data['datetime'].astype(str), format='%Y%m%d', errors='coerce').dt.strftime(
            '%Y-%m-%d'
        )
    for column in ['market', 'category', 'datetime', 'liutong_before', 'total_before', 'liutong_after', 'total_after']:
    ...
    return data[columns].sort_values(['code', 'datetime']).reset_index(drop=True)
