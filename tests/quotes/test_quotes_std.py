...
class TestStdTransactions(unittest.TestCase):
    def test_transactions_formats_history_ticks(self):
        class FakeClient:
            def __init__(self):
                self.kwargs = None
            ...
            def get_history_transaction_data(self, **kwargs):
                self.kwargs = kwargs
                return [
                    {'time': '14:56', 'price': 9.32, 'vol': 33, 'buyorsell': 0},
                    {'time': '14:56', 'price': 9.32, 'vol': 30, 'buyorsell': 1},
                ]

        client = StdQuotes.__new__(StdQuotes)
        client.client = FakeClient()

        data = client.transactions(symbol='000001', start=10, offset=2, date='20170209')

        self.assertEqual(
            client.client.kwargs,
            {'market': 0, 'code': '000001', 'start': 10, 'count': 2, 'date': 20170209},
        )
        self.assertEqual(list(data['order']), [10, 11])
        self.assertEqual(list(data['code']), ['000001', '000001'])
        self.assertEqual(list(data['date']), ['20170209', '20170209'])
        self.assertEqual(str(data.index[0]), '2017-02-09 14:56:00')
